          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.active) {
              if (snapshot.hasData) {
                ...
                print(transcriptsUrlName['school transcripts name'].runtimeType);
                if (transcriptsUrlName!= null) {
                  transcriptFilenames = transcriptsUrlName['school transcripts name'].cast<String>();
                  transcriptUrls = transcriptsUrlName['school transcripts urls'].cast<String>();
                }
              }
              if (transcriptsUrlName!= null) {
                transcriptFilenames = transcriptsUrlName['school transcripts name'].cast<String>();
...
//                                      )));
          ...
          child: Container(
            ...
            child: GestureDetector(
              onTap: () async {
                ...
                widget.userBloc.deleteTranscriptFile(
                    widget.userId, filename);
                transcriptUrls = await widget.userBloc
                    .getOnlyTranscriptsUrl(
                    widget.userId,
                    transcriptFilenames);
                widget.userBloc
                    .registerSchoolTranscripts(
                    widget.userId, transcriptUrls,
                    transcriptFilenames);
  ...
  Future<void> uploadTranscripts(List<File> files) async {
    {
      widget.userBloc = BlocProvider.of(context);
      try {
        transcriptUrls = await widget.userBloc.getUploadTranscriptsUrl(
            widget.userId, transcriptFilenames, files);
        print('Aqui');
